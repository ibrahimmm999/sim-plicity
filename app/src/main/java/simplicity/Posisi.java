...
public class Posisi {
    ...
    public Posisi(Point kiriAtas, Point kananBawah) {
        if (kiriAtas.getX() <= kananBawah.getX() && kiriAtas.getY() <= kananBawah.getY()) {
            ...
            System.out.println(
                    "Perhatikan kembali posisi titik kiri atas dan kanan bawah: nilai x pada koordinat kiri atas harus lebih kecil atau sama dengan nilai x pada koordinat anan bawah, sedangkan nilai y pada koordinat kiri atas harus lebih besar atau sama dengan nilai y pada koordinat kanan bawah.");
        }
    }
